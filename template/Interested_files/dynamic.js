...
Calendar._C = null;
Calendar.is_ie = (/msie /
    i.test(navigator.userAgent) && !/opera /
  ...
  if (this.showsTime) {
    ...
    return part;
      ...
      cal.onUpdateTime = function () {
        ...
        if (t12) {
          if (/pm /
              i.test(AP.innerHTML) && h < 12) h += 12;
          else if (/am /
              i.test(AP.innerHTML) && h == 12) h = 0;
        }
        var d = date.getDate();
        var m = date.getMonth();
    ...
    for (var j = 0; j < 7; ++j, cell = cell.nextSibling, date.setDate(iday + 1)) {
      ...
      if (this.getDateStatus) {
        ...
        if (status == = true) {
          ...
          if (/disabled /
              i.test(status)) cell.disabled = true;
