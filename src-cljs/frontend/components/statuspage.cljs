...
(defn incident-markup [incident]
  (if-let [updates (:incident_updates incident)]
    (html [:p
           [:a {:href (str "https://" page ".statuspage.io")}
            (str (:impact incident) " incident")]
           (str " created at " (:created_at incident) " - updated at " (:updated_at (first updates)))
           [:br]
           (str (:body (first updates)))])
    (html [:p
           (str (:impact incident) " incident created at " (:created_at incident))])))
