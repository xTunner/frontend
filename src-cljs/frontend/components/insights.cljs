...
(defn project-insights [{:keys [show-insights? reponame username branches recent-builds chartable-builds sort-category parallel insights-selected-branch] :as project} owner]
  (reify
    om/IDidMount
    (did-mount [_]
    ...
    (render [_]
      (html
       (let [branch insights-selected-branch
         ...
         [:div.project-block {:class (str "build-" (name sort-category))}
          ...
          (cond (nil? (get recent-builds insights-selected-branch)) [:div.loading-spinner common/spinner]
...
(defn decorate-project
  ...
  (let [chartable-builds (filter-chartable-builds (get recent-builds (:insights-selected-branch project)))]
    (-> project
