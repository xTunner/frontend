...
(dc/do
  ...
  (defcard link-buttons
    "These are our link buttons, when you need to style a link as a button. There are `:medium` size link buttons to be used in table rows and anywhere vertical space is at a premium.

    A link-button's label, like an ordinary button's, should be an action—that is, an imperative verb. Like a normal button, clicking it initiates that action. Clicking a link-button in particular \"initiates\" the action by navigating to a place in the app where the user can continue the action. For instance, \"Add Projects\" is a link, because it navigates to the Add Projects page, but it is a link-button in particular because it takes the user there to perform the \"Add Projects\" action.

    Viewing more information is not an action. \"Build #5\" would not be an appropriate label for a link-button; neither would \"View Build #5\". Instead, \"Build #5\" should be a normal link."
