...
(defmethod post-control-event! :upload-p12
  [_ _ {:keys [project-name file-content file-name password description]} previous-state current-state]
  (let [uuid frontend.async/*uuid*
        api-ch (get-in current-state [:comms :api])]
    (api/set-project-code-signing-keys project-name file-content file-name password description api-ch uuid)))
