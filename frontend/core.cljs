...
(defn main [state top-level-node]
  (let [comms       (:comms @state)
    ...
    (go (while true
          (alt!
           (:controls comms) #(controls-handler % state container)
           (:nav comms) #(nav-handler % state history-imp)
           (:api comms) #(api-handler % state container)
           (:ws comms) #(ws-handler % state pusher-imp)
