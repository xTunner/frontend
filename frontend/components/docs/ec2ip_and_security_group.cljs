(ns frontend.components.docs.ec2ip-and-security-group)

(def article
  {:title "Circle's EC2 IP addresses and AWS security group"
   :last-updated "Jun 19, 2013"
   :url :ec2ip-and-security-group
   :content [:div
             [:p
             ...
             [:p
              ...
              [:a
               {:href "https://forums.aws.amazon.com/ann.jspa?annID=1701"}
               "EC2 public IP address ranges"]
              "are there.(Be aware that this link can sometimes be broken when Amazon make updates.In such cases, you can usually access the list via the service's"
              [:a
               {:href "https://forums.aws.amazon.com/forum.jspa?forumID=30"}
               "forum announcements"]
              "page.)"]
             [:p
