...
(defn branch [project [name-kw branch]]
  (let [display-builds (take 5 (sort-by :build_num (concat (:running_builds branch)
                                                           (:recent_builds branch))))]
    [:div.aside-project-branch
     [:a.aside-name
     ...
     [:div.aside-status-icons
      {:data-bind
       "foreach: {data: project.recent_branch_builds(branch_name), as: 'recent'}"}
       ...
       (let [user     (:user data)
         ...
         [:div.aside-project
          [:div.aside-project-info
           ...
           (when (project-model/show-toggle-branches? user project)
             [:a.toggle-all
              {:role "button",
          ...
          (map (partial branch project)
               (if (get-in settings [:projects project-id :show-all-branches])
                 (:branches project)
                 (project-model/personal-branches user project)))])))))
      ...
      (let [controls-ch (get-in opts [:comms :controls])
        ...
        (html/html
         [:aside
          ...
          [:div.aside-projects
           (map #(om/build project {:project %
                                    :user user
                                    :ch controls-ch
                                    :settings (:settings app)}) (:projects app))]]])))))
